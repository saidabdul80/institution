...
class Utilcopy
{
    ...
    public static function canPerformEnrollment($user, $req){
        $query = VendorEnrollmentPoint::query();
    
        if ($user instanceof Vendor && $req) {            
            return $query->lgaIds($user->id)->contains($req->lga_id);
        }
    
        if ($user instanceof Agent && $req) {
            return $query->lgaIds($user->vendor_id)->contains($req->lga_id);
        }
    
        return false; 
    }

    public static function canCollectPayment($user, $req)
    {
        if ($user instanceof Vendor) {
            $res = VendorCollectionPoint::withRevenueSource($user->id,$req);            
        } elseif ($user instanceof Agent) {
            $res = VendorCollectionPoint::withRevenueSource($user->vendor_id, $req);
        } else {
            return true; // allow
        }        
        return $res;
    ...
    public static function verifyOtp($request, $user_type){
        ...
        if($user_type === 'individual'){
            $user =  IndividualTaxPayer::where('email', $request->email)->where('otp', $request->otp)->where('otp_expires_at', '>', now())->first();                    
        }elseif($user_type === 'corporate'){
            $user = CorporateTaxPayer::where('email', $request->email)->where('otp', $request->otp)->where('otp_expires_at', '>', now())->first();        
        }
    ...
    //                 if ($invoices->count() == 1) {
    //                     $invoice->update([
    //                         'payment_status' => PaymentStatus::PART_PAID
    //                     ]);
    //                 } else {
    //                     $wallet = Wallet::getWalletWithInvoice($invoice);
    ...
    //     }
    // }
    static public function processPaymentCompletion($payment, $totalPaid = 0, $date= null) {    
     
        $amountUsedOnAlreadyMarkedInvoices = $payment->taxInvoices
                    ->where('payment_status', PaymentStatus::PAID)
                    ->sum('amount');
        $totalPaid = max(0, $totalPaid - $amountUsedOnAlreadyMarkedInvoices);
        $invoices = $payment->taxInvoices->filter(fn($invoice) => $invoice->payment_status === PaymentStatus::UNPAID ||  $invoice->payment_status === PaymentStatus::PART_PAID);
       ...
       foreach ($invoices as $index => $invoice) {
           if ($totalPaid <= 0) {
            ...
            $isLastInvoice = $invoiceCount > 0? ($index == $invoiceCount - 1): false;
            $paidAmount = null;
            if ($totalPaid >= $invoice->amount) {
                
                
                self::revenueSubHeadResolver($invoice, $invoice->amount);
                $invoice->update(['payment_status' => PaymentStatus::PAID, 'paid_amount' => $invoice->amount]);

                $totalPaid -= $invoice->amount;
                $totalAmountUsed += $invoice->amount; 
              
                // if ($isLastInvoice && $totalPaid > 0 && $payment->gateway == 'paystack') {
                //     Wallet::getWalletWithInvoice($invoice)->credit($totalPaid, 'Leftover: Witholding ');
                // }
                Log::info("state 2: " . $invoice->id);
            } else {
                $totalAmountUsed += $totalPaid;
                $paidAmount = $totalPaid + floatval($invoice->paid_amount ?? 0);
                $updateData['paid_amount'] = $paidAmount;
                Log::info("state 3: " . $invoice->id);
                if($paidAmount >= $invoice->amount){
                    Log::info("state 4: " . $invoice->id);
                    $updateData['paid_amount'] = $invoice->amount;
                    $updateData['payment_status'] = PaymentStatus::PAID;
                    self::revenueSubHeadResolver($invoice, $invoice->amount);
                }else{
                    Log::info("state 5: " . $invoice->id);
                    // Determine the payment status based on the paid amount
                    $updateData['payment_status'] = PaymentStatus::PART_PAID;
                }
                // Update the invoice with the calculated values
                $invoice->update($updateData);
                
                // Reset $totalPaid to 0
                $totalPaid = 0;
            }
            $invoicePayments[$invoice->id] = $paidAmount ?? $invoice->amount;
        }

        $payment->update([
            'paid_amount' => $totalAmountUsed,
            'paid_time' => Carbon::now()->format('H:i:s'),
            //if true, payment is paid by reference, and date is the date of the payment, i.e date on gateway
            'date'=> $date ?? Carbon::now()->format('Y-m-d')
        ]);

        if (!empty($invoicePayments)) {
            $invoiceIds = array_keys($invoicePayments);
            //$invoiceIds = $invoices->pluck('id'); // Get all invoice IDs
            $initiatedInvoices = InitiatedPaymentTaxInvoice::where('payment_id', $payment->id)
                ->whereIn('tax_invoice_id', $invoiceIds)
                ->get()
                ->keyBy('tax_invoice_id'); // Index by tax_invoice_id for quick lookup

            foreach ($invoicePayments as $invoiceId => $paidAmount) {
                if (isset($initiatedInvoices[$invoiceId])) {
                    $initiatedInvoices[$invoiceId]->update([
                        'paid_amount' => $paidAmount,
                        'paid_date' => $date ?? Carbon::now()->format('Y-m-d')
                    ]);
                }
            }
        }
    ...
    static public function revenueSubHeadResolver($invoice, $paidAmount)
    {
        ...
        switch ($revenueSubHead->revenue_type) {
            ...
            case RevenueType::PAYE:
                CorporateTaxPayerEmployee::where('id', $invoice->related_id)->update(['payment_status'=>PaymentStatus::PAID,'tax_amount'=> $paidAmount]);
                break;
            case RevenueType::ASSETS:
                $taxableAsset = TaxableBusinessAsset::find($invoice->related_id);
                $complianceDate = BusinessAssetsCompliance::where('taxable_business_asset_id', $invoice->related_id)
                                ->latest('compliance_date')->first();

                if (empty($taxableAsset)) {
                ...
                $lastComplianceDate = is_null($complianceDate?->compliance_date) ? Carbon::parse($taxableAsset->created_at) : Carbon::parse($complianceDate->compliance_date);
            ...
            CASE RevenueType::TICKETS:
                ...
                for ($i = 0; $i < $ticket->total_ticket; $i++) {
                    $tickets[] = [
                        'ticket_id' => $ticket->id,
                        'ticket_number' => $ticket->code . '/' . Str::random(5),
                        'generator_type' => get_class($ticket->vendor),
                        'generator_id' => $ticket->vendor->id,
    ...
    //     $invoices = $payment->taxInvoices;
    //    //dd(self::splitPayments($invoices, $amount)) ;
    //    $paymentSpliting= collect([]);
    //     $subaccounts = self::splitPayments($invoices, $amount);
    //     $prep_subaccounts = $subaccounts->map(function ($subaccount) {
    //         $subaccount_code = app(BeneficiaryController::class)->updateSubaccount($subaccount,$subaccount['share']);
    ...
    //     $prep_subaccounts = $prep_subaccounts->filter(function ($subaccount) {
    //         return $subaccount !== null;
    //     });
        

    //     //change if share is not equal to 100, add the remaining amount to the last subaccount
    //     $subaccounts[$subaccounts->count()-1]['share'] = self::lastSubaccountShare($subaccounts);
    //    //prepare paymentSpliting
       
    //     PaymentSpliting::create($paymentSpliting->toArray());

    //     $response = Http::withHeaders($header)->post("https://api.paystack.co/split", [
    //         'name' =>'Halfsies',
    //         'type' => "percentage",
    //         'currency' => "NGN",
    //         'subaccounts' =>  $prep_subaccounts->toArray(),
    //     ]);
    ...
    //             return [
    //                 "id" => $subaccount['id'], // use the original subaccount
    //                 "subaccount" => $subaccount['subaccount'], // use the original subaccount
    //                 "share" => $subaccount['share'],
    //                 "collected_share" => 0
    //             ];
    //         }
    ...
    //     // Filter out subaccounts with collected_share as 0 for the API request
    //     $filtered_subaccounts = $prep_subaccounts->filter(function ($subaccount) {
    //         return $subaccount['collected_share'] != 0;
    //     })->toArray();

    //     // Adjust the share of the last subaccount if the total share is not 100
    //     $last_share = self::lastSubaccountShare(collect($filtered_subaccounts));
    ...
    //     // Send request to Paystack API with filtered subaccounts
    //     $response = Http::withHeaders($header)->post("https://api.paystack.co/split", [
    //         'name' => 'Halfsies',
    //         'type' => "percentage",
    //         'currency' => "NGN",
    //         'subaccounts' => collect($filtered_subaccounts)->map(function ($subaccount) {
    //             return [
    ...
    private static function splitPayments($invoices, $amountToPay,$gateway){
        ...
        //dd($invoices->pluck('amount')->toArray());
        foreach ($invoices as $invoice) {
            $invoiceAmount = $invoice->amount;
