...
class CreateStudentSemesterGpaTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('student_semester_gpa', function (Blueprint $table) {
            ...
            // Indexes
            $table->index(['student_id', 'session_id', 'semester'], 'student_session_semester_index');
            $table->unique(['student_id', 'session_id', 'semester'], 'student_session_semester_unique');
            ...
            $table->foreign('compiled_by')->references('id')->on('staffs')->onDelete('set null');
        });
    }
